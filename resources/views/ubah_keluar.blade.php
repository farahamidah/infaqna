...
<body>
    ...
    <form action="{{route('infaqkeluar.update', $infaqkeluar->id)}}" method="POST">
    @csrf
    @method('PUT')
    <ul class="list-group">
    <!-- No <input type="text" name="no" required value="{{$infaqkeluar->no}}">    -->
    Keterangan <input class="mt-1" type="text" name="keterangan" required value="{{$infaqkeluar->keterangan}}">
    Tanggal <input class="mt-1" type="date" name="tanggal" required value="{{$infaqkeluar->tanggal}}">
    Jumlah <input class="mt-1" type="text" name="jumlah" required value="{{$infaqkeluar->jumlah}}">
